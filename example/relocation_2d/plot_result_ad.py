...
maxiter = [161, 30, 30, 30]
label = ['ad_p'] #, 'ad_ps', 'ad_p_more', 'ad_ps_more']
t0s = [True, False, False, False]
# mms = [1.0e-8, 1.0e-5, 1.0e-4, 1.0e-4]

make_directory('./plot')

station = read_array('./geometry/station.txt', (nr, 2), ascii=True)
vp = read_array('./model/vp.bin', (301, 301))
mask = read_array('./model/mask.bin', (301, 301))
vp[mask == 0.0] = np.nan
...
for itest in range(len(label)):
    ...
    iter = maxiter[itest]
    iters = [5, 10, iter]
    dir = 'reloc_' + label[itest]
    output = './plot/' + label[itest]
    has_t0 = t0s[itest]
    ...
    # subprocess.Popen("mkdir -p tloc", shell=True)
    ...
    # p_obs = read_array(dir + '/record_processed/shot_' + num2str(shot) + '_traveltime_p.bin', nr) - t_obs
    # if has_s:
    #     s_obs = read_array(dir + '/record_processed/shot_' + num2str(shot) + '_traveltime_s.bin', nr) - t_obs

    if has_t0:
        t_obs = read_array('./model/st0.bin', ns)
        delta = np.zeros(ns)
        for i in range(ns):
            t_p = read_array('./data/shot_' + str(i + 1) + '_traveltime_p.bin', nr)
            delta[i] = np.min(t_p)
        t_init = read_array(dir + '/iteration_0/model/st0.bin', ns) + delta
        t_final = read_array(dir + '/iteration_' + num2str(iter) + '/model/updated_st0.bin', ns) + delta - 3
    ...
    # else:
    #     t_init = np.zeros(nr)
    # p_init = read_array(dir + '/iteration_0/synthetic/shot_' + num2str(shot) + '_traveltime_p.bin', nr) - t_init
    # if has_s:
    #     s_init = read_array(dir + '/iteration_0/synthetic/shot_' + num2str(shot) + '_traveltime_s.bin', nr) - t_init

    # if has_t0:
    #     
    # else:
    #     t_final = np.zeros(nr)
    # p_final = read_array(dir + '/iteration_' + num2str(iter) + '/synthetic/shot_' + num2str(shot) + '_traveltime_p.bin', nr) - t_final
    # if has_s:
    #     s_final = read_array(dir + '/iteration_' + num2str(iter) + '/synthetic/shot_' + num2str(shot) + '_traveltime_s.bin', nr) - t_final
    ...
    ax.scatter(station[:, 0], station[:, 1], s=5, c='k', marker='v', edgecolors='k')
    for i in range(ns):
        ax.arrow(x_obs[i], z_obs[i], x_init[i] - x_obs[i], z_init[i] - z_obs[i], color='gray', alpha=0.75, linewidth=0.5)
    ax.scatter(x_obs, z_obs, s=10, c='b', marker='o', label='Ground Truth')
    ax.scatter(x_init, z_init, s=10, c='r', marker='o', label='Initial Guess')
    ...
    ax.legend(ncols=2)
    ax.imshow(vp, alpha=0.5, extent=[0, 3000, 0, 3000], origin='lower', cmap='Spectral_r')
    plt.tight_layout()
    ...
    ax.scatter(x_obs, z_obs, s=10, c='b', marker='o', label='Ground Truth')
    ax.scatter(x_final, z_final, s=10, c='r', marker='o', label='Inverted')
    ...
    ax.imshow(vp, alpha=0.5, extent=[0, 3000, 0, 3000], origin='lower', cmap='Spectral_r')
    plt.tight_layout()
    plt.savefig(output + '_final.pdf', dpi=300, bbox_inches='tight', pad_inches=2.0 / 72.0)
    ...
    if has_t0:
        ...
        # ax.plot(regspace(0.0, ns - 1.0, 1.0), t_obs, 'b', label='Ground Truth')
        # ax.plot(regspace(0.0, ns - 1.0, 1.0), t_init, 'lime', label='Initial Guess')
        # ax.plot(regspace(0.0, ns - 1.0, 1.0), t_final, 'r', label='Inverted')

        ax.plot(regspace(0.0, ns - 1.0, 1.0), t_init - t_obs, 'b')
        ax.plot(regspace(0.0, ns - 1.0, 1.0), t_final - t_obs, 'r')
        ax.scatter(regspace(0.0, ns - 1.0, 1.0), t_init - t_obs, s=10, c='b', label='Initial Misfit')
        ax.scatter(regspace(0.0, ns - 1.0, 1.0), t_final - t_obs, s=10, c='r', label='Final Misfit')

        ax.set_xlim([0, ns - 1.0])
        ax.set_ylim([-5, 5])
